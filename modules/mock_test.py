# ============================================================
# modules/mock_test.py
# Final 21-Day Challenge Mock Test - All 4 Skills
# ============================================================
...
SPEAKING_QUESTIONS = [
    ...
    "Some people believe technology is making people less social. Do you agree or disagree? Give reasons and examples.",
]
...
Paragraph A
Every day, humans make thousands of decisions, from minor choices like what to eat for breakfast to major life decisions such as career changes or financial investments. Psychologists have long studied the mechanisms behind these choices, revealing that human decision-making is far more complex and irrational than previously assumed.

Paragraph B
Daniel Kahneman, a Nobel Prize-winning psychologist, proposed that humans operate using two distinct cognitive systems. System 1 is fast, automatic, and emotional, while System 2 is slow, deliberate, and logical. Most everyday decisions are driven by System 1, which relies heavily on cognitive shortcuts known as heuristics. While these shortcuts are efficient, they can lead to systematic errors in judgment.

Paragraph C
One of the most documented cognitive biases is the confirmation bias, whereby individuals tend to search for, interpret, and recall information in ways that confirm their existing beliefs. Studies have shown that this bias affects decisions in fields ranging from medicine to finance. A doctor who initially suspects a patient has a particular illness may unconsciously disregard symptoms that suggest an alternative diagnosis.

Paragraph D
Another significant factor in decision-making is the concept of loss aversion. Research indicates that people feel the pain of losing something approximately twice as strongly as they feel the pleasure of gaining something of equal value. This asymmetry has profound implications for financial behaviour, causing investors to hold onto losing stocks far longer than is rational.

Paragraph E
Environmental factors also play a crucial role. A study conducted in 2011 found that judges were significantly more likely to grant parole in the morning and directly after breaks, with approval rates dropping to nearly zero by the end of each session. This phenomenon, known as decision fatigue, demonstrates that the quality of decisions deteriorates as cognitive resources become depleted.

Paragraph F
Despite these limitations, humans have developed strategies to improve decision-making. Breaking complex decisions into smaller components, seeking diverse perspectives, and introducing deliberate delays before committing to major choices have all been shown to improve outcomes. The growing field of behavioural economics applies these insights to design better policies and systems."""

READING_QUESTIONS = [
    ("TRUE/FALSE/NOT GIVEN", "Daniel Kahneman received a Nobel Prize for his work on decision-making.", "TRUE"),
    ("TRUE/FALSE/NOT GIVEN", "System 2 thinking is faster than System 1 thinking.", "FALSE"),
    ...
    ("SHORT ANSWER", "What term describes the cognitive shortcuts humans use? (max 3 words)", "heuristics"),
    ("SHORT ANSWER", "Which paragraph discusses environmental influences on decisions?", "paragraph e"),
    ("SHORT ANSWER", "What field applies insights from decision-making research to policy design?", "behavioural economics"),
]

LISTENING_SCRIPT = """IELTS Listening Practice - Section 2
...
LISTENING_QUESTIONS = [
    "In what year was the Riverside Community Centre established?",
...
def render_mock_test():
    """Render the full 4-skill final mock test."""
    ...
    if stage == "intro":
        _render_intro(profile, accent, target_band)
    elif stage == "speaking":
        _render_speaking(profile, accent)
    elif stage == "writing":
        _render_writing(profile, accent)
    elif stage == "reading":
        _render_reading(profile, accent)
    elif stage == "listening":
        _render_listening(profile, accent)
    elif stage == "results":
        _render_results(profile, accent, target_band)
...
def _render_intro(profile, accent, target_band):
    ...
    <div class="glass-card" style="text-align:center;padding:48px 32px">
        ...
        <div style="font-size:15px;color:rgba(255,255,255,0.5);line-height:1.7;max-width:520px;margin:0 auto 28px">
            You have completed 21 days of training. This final test covers all 4 IELTS skills.
            Score <strong style="color:{accent}">Band {target_band}+</strong> to earn your
            <strong style="color:{accent}">Gold Achievement Certificate</strong>.
        </div>
        <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:12px;max-width:380px;margin:0 auto 28px">
            <div style="background:rgba(167,139,250,0.1);border:1px solid rgba(167,139,250,0.3);border-radius:12px;padding:14px">
                ...
                <div style="font-size:11px;color:rgba(255,255,255,0.35);margin-top:2px">3 questions</div>
            </div>
            <div style="background:rgba(56,189,248,0.1);border:1px solid rgba(56,189,248,0.3);border-radius:12px;padding:14px">
                ...
                <div style="font-size:11px;color:rgba(255,255,255,0.35);margin-top:2px">Task 1 + Task 2</div>
            </div>
            <div style="background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);border-radius:12px;padding:14px">
                ...
                <div style="font-size:11px;color:rgba(255,255,255,0.35);margin-top:2px">10 questions</div>
            </div>
        ...
        <div style="background:rgba(240,192,64,0.08);border:1px solid rgba(240,192,64,0.2);border-radius:12px;padding:14px;max-width:380px;margin:0 auto 24px;font-size:13px;color:rgba(255,255,255,0.6)">
            Gold Certificate: Score Band {target_band}+ overall<br>
...
def _render_speaking(profile, accent):
    ...
    <div style="font-size:13px;color:rgba(255,255,255,0.4);margin-bottom:20px">
        Answer all 3 questions as fully as possible. Aim for 3-4 sentences minimum per answer.
    </div>
    """, unsafe_allow_html=True)
    ...
    for i, q in enumerate(SPEAKING_QUESTIONS):
        ...
        if i == q_idx and not answered:
            answer = st.text_area(
                ...
                height=100,
                key=f"spk_input_{i}",
                placeholder="Type your answer here — aim for at least 3-4 sentences..."
            )
    ...
    all_answered = all(f"speaking_q{i}" in st.session_state.mock_answers for i in range(3))
    if all_answered:
        st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
        st.markdown('<div class="btn-primary">', unsafe_allow_html=True)
        if st.button("Score Speaking and Continue", key="score_speaking"):
            with st.spinner("AI is scoring your Speaking answers..."):
...
def _render_writing(profile, accent):
    ...
    <div style="font-size:13px;color:rgba(255,255,255,0.4);margin-bottom:20px">
        Complete both tasks. Task 1: minimum 150 words. Task 2: minimum 250 words.
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.expander("Task 1 — Graph Description" + (" ✅" if t1_done else ""), expanded=not t1_done):
        ...
        if not t1_done:
            ...
            st.caption(f"Word count: {wc1} / 150 minimum")
    ...
    with st.expander("Task 2 — Essay" + (" ✅" if t2_done else ""), expanded=t1_done and not t2_done):
        ...
        if not t2_done:
            ...
            st.caption(f"Word count: {wc2} / 250 minimum")
    ...
    if t1_done and t2_done:
        ...
        if st.button("Score Writing and Continue", key="score_writing"):
            with st.spinner("AI is scoring your Writing responses..."):
...
def _render_reading(profile, accent):
    ...
    st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
    st.markdown(f"""
    <div style="font-size:14px;font-weight:700;color:#34D399;margin-bottom:12px">Questions</div>
    """, unsafe_allow_html=True)
...
def _render_listening(profile, accent):
    ...
    <div style="font-size:13px;color:rgba(255,255,255,0.4);margin-bottom:16px">
        Read the script ONCE carefully. Then answer all 10 questions from memory without looking back.
    </div>
    """, unsafe_allow_html=True)
...
def _render_results(profile, accent, target_band):
    ...
    if achieved:
        ...
        <div style="background:linear-gradient(135deg,rgba(240,192,64,0.15),rgba(240,192,64,0.05));
            ...
            <div style="font-size:26px;font-weight:800;color:{accent};margin-bottom:8px">
                Congratulations! Target Achieved!
            </div>
            <div style="font-size:15px;color:rgba(255,255,255,0.6);margin-bottom:8px">
                You scored <strong style="color:{accent}">Band {overall}</strong> — your target was Band {target_band}.
            </div>
            <div style="font-size:13px;color:rgba(255,255,255,0.4)">
                Your Gold Achievement Certificate is ready to download below.
            </div>
        </div>
        """, unsafe_allow_html=True)
    else:
        ...
        <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);
            ...
            <div style="font-size:13px;color:rgba(255,255,255,0.4)">
                Keep training and retry the challenge to earn your Gold Certificate.
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    for col, (skill, score, color) in zip([c1, c2, c3, c4], skill_data):
        with col:
            st.markdown(f"""
    ...
    label = "🏆 Download Gold Achievement Certificate" if achieved else "🥈 Download Completion Certificate"
    st.download_button(
        label=label,
        data=cert_html,
        file_name=f"IELTS_Master_Certificate_{profile.get('full_name','Student').replace(' ','_')}.html",
        mime="text/html",
        use_container_width=False,
        key="dl_final_cert"
    )
    st.caption("Open the downloaded .html file in any browser to view, print, or screenshot your certificate.")
    ...
    if user_id != "demo" and scores and not st.session_state.get("mock_scores_saved"):
...
def _score_speaking(profile) -> float:
    ...
    combined = ""
    for i, q in enumerate(SPEAKING_QUESTIONS):
        ans = answers.get(f"speaking_q{i}", "")
        combined += f"Q{i+1}: {q}\nAnswer: {ans}\n\n"

    system = build_base_prompt(profile) + """
You are an IELTS Speaking examiner. Score these 3 answers holistically.
Return ONLY a single number between 3.0 and 9.0 in 0.5 increments.
Consider: Fluency, Lexical Resource, Grammar, Pronunciation clues.
Return format: just the number, e.g. "6.5"
"""
    response = chat([{"role": "user", "content": combined}], system, max_tokens=10)
    return _extract_score(response, default=5.5)
...
def _score_writing(profile) -> float:
    ...
    system = build_base_prompt(profile) + """
You are an IELTS Writing examiner. Score both tasks holistically.
Task 1 prompt: Line graph about owned vs rented accommodation in England.
Task 2 prompt: Prison sentences vs alternative crime reduction methods.
Return ONLY a single number between 3.0 and 9.0 in 0.5 increments.
Return format: just the number, e.g. "6.0"
"""
    content = f"TASK 1:\n{t1}\n\nTASK 2:\n{t2}"
...
def _score_reading() -> float:
    correct = 0
    for i, (_, _, correct_answer) in enumerate(READING_QUESTIONS):
        user_answer = st.session_state.mock_answers.get(f"reading_q{i}", "").strip().lower()
        if user_answer == correct_answer.lower():
            correct += 1
    score_map = {10: 9.0, 9: 8.5, 8: 8.0, 7: 7.5, 6: 7.0,
                 5: 6.5, 4: 6.0, 3: 5.5, 2: 5.0, 1: 4.5, 0: 4.0}
    return score_map.get(correct, 5.0)


def _score_listening() -> float:
    correct = 0
    for i, correct_answer in enumerate(LISTENING_ANSWERS):
        user_answer = st.session_state.mock_answers.get(f"listening_q{i}", "").strip().lower()
        if correct_answer.lower() in user_answer or user_answer in correct_answer.lower():
            correct += 1
    score_map = {10: 9.0, 9: 8.5, 8: 8.0, 7: 7.5, 6: 7.0,
                 5: 6.5, 4: 6.0, 3: 5.5, 2: 5.0, 1: 4.5, 0: 4.0}
    return score_map.get(correct, 5.0)
