...
def render_dashboard():
    """Render the main user dashboard / HQ."""
    ...
    band_display = f"{baseline}" if baseline else "Not assessed"
    progress_pct = min(100, int(((baseline or 0) / target) * 100)) if baseline else 0
    ...
    # ── BOTTOM ROW: Recurring Errors + Recent Sessions ──
    ...
    with err_col:
        ...
        if errors:
            ...
            st.markdown("""
            <div style="font-size:13px;color:rgba(255,255,255,0.3);text-align:center;padding:20px 0">
    ...
    with sess_col:
        ...
        if recent:
            for s in recent:
                ...
                icon = mode_icon.get(skill, "🎓")
                band = s.get("overall_band", "-")
                band_color = "#2ECC71" if (band != "-" and float(band) >= 7) else "#F0C040"
                created = s.get("created_at", "")[:10] if s.get("created_at") else "Today"
